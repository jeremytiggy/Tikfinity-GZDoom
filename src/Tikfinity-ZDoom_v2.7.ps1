function Import-HashtableDirectory
{
    param
    (
    ...
    # Use ordered dictionary to preserve key order globally
    $combinedHashtable = [ordered]@{}

    $dataFiles = Get-ChildItem -Path $DirectoryPath -Filter '*.psd1' -File

    foreach ($file in $dataFiles)
    {
        # Read the entire file as raw PowerShell code.
        # Files now contain [ordered]@{} at every level, so we evaluate directly.
        # This is safe because you are the developer and control the .psd1 files.
        $fileContent = Get-Content -Raw $file.FullName
        $importedHashtable = Invoke-Expression -Command $fileContent

        # Merge into the combined ordered dictionary
        foreach ($key in $importedHashtable.Keys)
        {
            # Direct assignment preserves the key's position if it already exists,
            # otherwise appends at the end (order of first file wins for each key).
            $combinedHashtable[$key] = $importedHashtable[$key]
        }
    }

    # Optionally export to a global variable
    if (-not [string]::IsNullOrWhiteSpace($OutputVariableName))
    {
        Set-Variable -Name $OutputVariableName -Value $combinedHashtable -Scope Global
    }
...
try {
    Write-Host "[REST_API_Actions] Loading action data files..."
    $Global:REST_API_Actions = Import-HashtableDirectory -DirectoryPath ".\actions"
}
catch {

    Write-Host "Failed to load external data file for REST_API_Actions. Quitting."
    Write-Host $_
